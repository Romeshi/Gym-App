          ...
          PopupMenuButton<String>(
            ...
            onSelected: (value) {
              if (value == 'logout') {
                ...
                Navigator.push(context, MaterialPageRoute(builder: (context) => const RegisterGymScreen()));
              }
            },
            itemBuilder: (context) => [
              if (role == UserRole.owner) const PopupMenuItem(value: 'register', child: Text('Register Gym')),
              const PopupMenuItem(value: 'logout', child: Text('Logout')),
              const PopupMenuDivider(),
              const PopupMenuItem(enabled: false, child: Text('Switch View (Dev)')),
              PopupMenuItem(onTap: () => navProvider.setRole(UserRole.member), child: const Text('  • Member View')),
              PopupMenuItem(onTap: () => navProvider.setRole(UserRole.trainer), child: const Text('  • Trainer View')),
              PopupMenuItem(onTap: () => navProvider.setRole(UserRole.owner), child: const Text('  • Owner View')),
            ],
          ),
    ...
    switch (role) {
      case UserRole.member:
        return [
          ...
          const BottomNavigationBarItem(icon: FaIcon(FontAwesomeIcons.dumbbell), label: 'Workouts'),
          const BottomNavigationBarItem(icon: Icon(Icons.restaurant_rounded), label: 'Diet'),
          const BottomNavigationBarItem(icon: Icon(Icons.analytics_rounded), label: 'Growth'),
          const BottomNavigationBarItem(icon: Icon(Icons.campaign_rounded), label: 'Notices'),
        ];
      case UserRole.trainer:
        return [
          const BottomNavigationBarItem(icon: Icon(Icons.dashboard_rounded), label: 'Home'),
          const BottomNavigationBarItem(icon: Icon(Icons.people_rounded), label: 'Clients'),
          const BottomNavigationBarItem(icon: Icon(Icons.assignment_rounded), label: 'Assign'),
